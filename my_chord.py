...
class ChordNode:
#-
  def __init__(self, chan,address):                    #- 
    ...
    self.nodeID  = int(self.chan.join('node',self.address)) # Find out who you are         #-
...
#- 
  def run(self): #-
    ...
    others = list(self.chan.members['node'] - set([str(self.nodeID)])) #-
    for i in others: #-
      self.addNode(i) #-
      self.chan.sendTo(nodeID,[i], (JOIN)) #-
    self.recomputeFingerTable() #-
 #-
    while True: #-
      message = self.chan.recvFromAny(nodeID) # Wait for any request #-
